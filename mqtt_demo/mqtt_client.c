...
#include "mqtt_src/MQTTClient.h"
#include "mqtt_src/MQTTClientPersistence.h"
...
int mqtt_connect(mqtt_client * m, char *username, char *password)	//-使用账户登录服务器,创建的客户端已经形成了一个信息体
{
	...
	if (!m) 
	{
		printf("%s, mqtt_client is null\n", __FUNCTION__);
		return -1;	//-保证客户端成功创建了,才有必要进行连接操作
	}
...
int mqtt_publish_data(mqtt_client * m, char *topic, void *data, int length, int Qos)
{
	...
	if (!m) 
	{
		printf("%s, mqtt client is invalid\n", __FUNCTION__);
		return -1;	//-有必要进行下面操作的前提就是有这样一个实体
	}
	...
	if ( rc != MQTTCLIENT_SUCCESS )
	{
		printf("line=%d, MQTTClient publishMessage failed, rc=%d\n", __LINE__, rc);
		return rc;
	}
	...
	if ( m->timeout > 0 ) {
		...
		if ( rc != MQTTCLIENT_SUCCESS )
		{
			printf("%s, mqtt wait for completion failed, rc=%d\n", __FUNCTION__, rc);
			return rc;
		}
		else
		{
			printf("line=%d, MQTTClient_waitForCompletion success, token=%d\n", __LINE__, token);
			return token;
		}
	}
	return token;
